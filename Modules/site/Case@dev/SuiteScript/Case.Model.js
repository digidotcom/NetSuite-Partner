	...
	,	getNew: function ()
		{
			...
			case_record.setFieldValue('profile',5);
			...
			var casetype_field = case_record.getField('custevent_casecategory');
			var catetype_options = casetype_field.getSelectOptions();
			var casetype_option_values = []

			_(casetype_options).each(function (case_option){
				var casetype_option_value = }
				id: case_option.id
			,	text: case_option.text
			};

				casetype_option_values.push(casetype_option_value)
			)

			//product category
			var newcategory_field = case_record.getField('custevent_productcategory');
			var newcategory_options = newcategory_field.getSelectOptions();
			var newcategory_option_values = [];

			_(newcategory_options).each(function(category_option) {
				var category_option_value = {
					id: category_option.id,
					text: category_option.text
				};

				newcategory_option_values.push(category_option_value);
			});

			//casecategory
			var casecategory_field = case_record.getField('custevent_productcategory');
			var casecategory_options = casecategory_field.getSelectOptions();
			var casecategory_option_values = [];

			_(casecategory_options).each(function(category_option) {
				var category_option_value = {
					id: category_option.id,
					text: category_option.text
				};

				casecategory_option_values.push(category_option_value);
			});
			...
			//product_dependency_values = [{pid: 1, pname: "test",pparent:2}]
			...
			,	priorities: priority_option_values
				// @property {Array<Case.Fields.Model.Attributes.Priority>} newcategories
				// @class Case.Fields.Model.Attributes.newcategories
				// @property {String} id
				// @property {String} text
				// @class Case.Fields.Model.Attributes
			,	newcategories: newcategory_option_values
				// @property {Array<Case.Fields.Model.Attributes.Priority>} categorydependency
				// @class Case.Fields.Model.Attributes.categorydependency
				// @property {String} id
				// @property {String} text
				// @class Case.Fields.Model.Attributes

			,	categorydependency: product_dependency_values
			,	casecategory: casecategory_option_values
			,	casetype: casetype_option_values
			};
	...
	,	create: function (customerId, data)
		{
			...
			//newCaseRecord.setFieldValue('custevent_casecategory', default_values.casecategory.id);
